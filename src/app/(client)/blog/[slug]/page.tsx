      ...
      <Container className="grid grid-cols-1 lg:grid-cols-4 gap-5">
        <div className="md:col-span-3">
          ...
          <div>
              ...
              <div className="flex items-center relative group cursor-pointer">
                {blog?.blogcategories?.map(
                  (item: { title: string }, index: number) => (
                    <p
                      key={index}
                      className="text-3xl font-extralight text-shop_light_blue hover:text-shop_white tracking-wider"
                    >
                      {item?.title}
                    </p>
                  )
                )}
                <span className="absolute left-0 -bottom-1.5  inline-block w-full h-0.5 group-hover:bg-shop_white hover:cursor-pointer hoverEffect" />
              </div>
              <p className="flex items-center gap-1 text-3xl text-shop_light_blue relative group hover:cursor-pointer hover:text-shop_white hoverEffect">
                <Pencil size={25} /> {blog?.author?.name}
                <span className="absolute left-0 -bottom-1.5  inline-block w-full h-2 group-hover:bg-shop_dark_green hoverEffect" />
              </p>
              <p className="flex items-center gap-1 text-3xl text-shop_light_blue relative group hover:cursor-pointer hover:text-shop_white hoverEffect">
                <Calendar size={15} />{" "}
            ...
            <h2 className="text-5xl my-5 text-shop_light_blue tracking-wider font-extralight">{blog?.title}</h2>
            <div className="flex flex-col">
              <div className="text-shop_light_blue tracking-wide">
                <div>
                  {blog.body && (
                    <PortableText
                      value={blog.body}
                      components={{
                        block: {
                          normal: ({ children }) => (
                            <p className="my-5 text-xl first:mt-0 last:mb-0">
                              {children}
                            </p>
                          ),
                          h2: ({ children }) => (
                            <h2 className="my-5 text-2xl font-medium tracking-tight text-gray-950 first:mt-0 last:mb-0">
                              {children}
                            </h2>
                          ),
                          h3: ({ children }) => (
                            <h3 className="my-5 text-xl font-medium tracking-tight text-gray-950 first:mt-0 last:mb-0">
                              {children}
                            </h3>
                          ),
                          blockquote: ({ children }) => (
      ...
      <div className="border-3 border-shop_light_blue p-5 rounded-md">
        ...
        <div className="space-y-2 mt-2">
          {categories?.map(({ blogcategories }, index) => (
            ...
            >
              <p className="text-lg text-shop_light_blue font-extralight">{blogcategories[0]?.title}</p>
              <p className="text-lg text-shop_light_blue font-extralight">{`(1)`}</p>
            </div>
          ))}
        </div>
      </div>
